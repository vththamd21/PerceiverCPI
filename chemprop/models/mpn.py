...
class SageEncoder(nn.Module):
    def __init__(self, args, atom_fdim: int):
        ...
        # Layer đầu tiên từ feature nguyên tử sang hidden
        self.convs.append(SAGEConv(atom_fdim, self.hidden_size))
        # Các layer GraphSAGE tiếp theo
        for _ in range(self.depth - 1):
            self.convs.append(SAGEConv(self.hidden_size, self.hidden_size))

    def forward(self, data):
        # data là đối tượng Batch từ torch_geometric
        ...
        # Global Pooling để tạo vector đại diện cho cả phân tử
        return global_mean_pool(x, batch)

class MPN(nn.Module):
    def __init__(self, args, atom_fdim: int = None):
        super(MPN, self).__init__()
        from chemprop.features import get_atom_fdim
        self.atom_fdim = atom_fdim or get_atom_fdim()
        self.encoder = SageEncoder(args, self.atom_fdim)
