...
def train(model: InteractionModel,
    ...
    for batch in tqdm(data_loader, total=len(data_loader), leave=False):
        ...
        # 4. Gán mol_batch chính là đối tượng đồ thị của PyG và đưa lên GPU
        mol_batch = batch.to(next(model.parameters()).device)
        # -------------------------------------------------------------------
